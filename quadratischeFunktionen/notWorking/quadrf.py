...
def main():
    ...
    if choice == 1:
        ...
        print("Grundform: y =" + (" " if a == 1 else (str(a) if a < 0 else " " + str(a))) + "x²" + b_term + c_term)

        # Scheitelpunktform: a * (x - h)² + v
        h_term = format_term(-vertex_x)
        print("Scheitelpunktform: y =" + (str(a) if a == 1 else ((str(a)) if a < 0 else " " + str(a))) + " * (x" + h_term + ")²" + format_term(vertex_y))

        # Nullstellenform: a * (x - x1)(x - x2)
        x1_term = format_term(-x1)
        x2_term = format_term(-x2)
        print("Nullstellenform: y =" + (str(a) if a == 1 else (str(a) if a < 0 else " " + str(a))) + " * (x" + x1_term + ") * (x" + x2_term + ")")
