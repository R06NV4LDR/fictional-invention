...
def parse_and_convert_equation(equation):
    ...
    # Handle forms like k*(X**2+mX+n)
    if "*" in equation and "(" in equation:
        outer_part, inner_part = equation.split("*", 1)
        a = float(outer_part)
        inner_part = inner_part.strip("()")
        if "X**2" in inner_part:
            inner_parts = inner_part.split("X**2")
    ...
    # Handle standard forms like ax**2+bx+c
    if "X**2" in equation:
...
while True:
    # This part handles user interaction and prints the solutions
    a, b, c = map(float, input("Enter coefficients A, B, C separated by commas: ").split(','))
